  ...
  > 
    ...
    <template v-slot:text>
      <v-form @submit.prevent="submit">
        <v-text-field
          id="ChainImportModal_chainLabel"
          outlined
          autocomplete="off"
          :label="$tc('label')"
...
export default {
  ...
  methods: {
    ...
    submit () {
      this.loading = true
      this.$store.dispatch('resources/doImportChain', { file: this.material, chainlabel: this.label })
        .then(() => {
          this.$notify.success({})
          this.$emit('saved')
        })
